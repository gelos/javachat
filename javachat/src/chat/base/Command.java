...
/**
 * The Class Chat Command. Commands to communicate between clients and chat
 * server. Used serialization for transfer command object on network.
 */
public class Command implements Serializable {
	 ...
	 * <p>
	 * If message empty or command not in list return {@link CommandName#CMDERR}
	 * command.
	 *
	 * @param messageSubString
	 *            the command string
	 */
			...
			case CMDPRVMSG: // process /PRVMSG DLM UDLM username list UDLM DLM message
				...
				if (commandStringSplitedByCMDDLM.length > 2
					...
					if (commandStringSplitedByCMDDLM[2].substring(0, CMDDLM.toString().length())
						...
						messageSubString = commandStringSplitedByCMDDLM[2].substring(CMDDLM.toString().length()); // get
																													// private
						// message
					} else {
					...
					for (String string : commandStringSplitedByCMDDLM) {
						if (!string.isEmpty()) {
							payloadSubString += ((payloadSubString.length() != 0) ? CMDULDLM.toString() : "")
									+ string.trim();
						}
					}
